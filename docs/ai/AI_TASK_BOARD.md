...
| 2026-06-15 | Antigravity | main | JoinWorkspaceFragment Real Session Fix | PrefsManager.kt, AuthRepository.kt, AuthViewModel.kt, JoinWorkspaceFragment.kt | **Status**: Completed.<br>**Testing Notes**: Removed hardcoded `user@example.com` / UUID. Email now persisted on login/signup. Session loaded from SharedPreferences via `getCurrentSession()` → `loadCurrentSession()`. Logcat verified with TAG `JoinWorkspaceFragment`. |
| 2026-06-15 | Antigravity | main | Edge Function Idempotency & Profile Fix | supabase/functions/accept-invitation/index.ts | **Status**: Completed.<br>**Testing Notes**: Fixed FK constraint violation by mirroring profiles and restructured logic to guarantee idempotent success for previously accepted invites. |
| 2026-06-15 | Antigravity | main | Post-Acceptance Navigation Flow | PrefsManager.kt, AuthViewModel.kt, JoinWorkspaceFragment.kt, fragment_join_workspace.xml | **Status**: Completed.<br>**Testing Notes**: Fixed post-acceptance workflow. App now checks for updated workspace memberships and immediately directs users to `MainActivity` while maintaining valid UI states for empty or redirecting conditions. |
